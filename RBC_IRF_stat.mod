% =================================================================== %
%  This computes the posterior stationary IRFs.
%
% =================================================================== %
% close all

// dynare RBC_IRF_stat.mod


% Load the names of model variables and parameters:
load('M_endo_names')

dates = 1960:0.25:2018.75; n_endo = length(M_endo_names) ;n_periods = length(dates); n_draws = 1200;
% |||load the draws|||:
addpath(genpath('AsEstRBC'));
load('AsEstRBC_param1.mat') % these are 1200 retained draws from the posterior
parametri_loop = stock;
smoothed_series_loop = zeros(n_endo,n_periods,n_draws);
load('AsEstRBC_smooth1.mat')
smoothed_series_loop1 = stock;
smoothed_series_loop(:,:,1:size(smoothed_series_loop1,3)) = smoothed_series_loop1;
load('AsEstRBC_smooth2.mat')
...
i_endo_amu = find(strcmp(M_endo_names  , 'a_mu'));
smoothed_mut_mean = mean(parametri_loop(:, 10))*mean(smoothed_series_loop(i_endo_amu,:,:),3 );


global higheffect_glob
higheffect = higheffect_glob;
if higheffect == 1;
    timedate = 60; % 1974:Q4
elseif higheffect == 0;
    timedate    =   183; % 2005:Q3
end

figure()
plot(dates  ,  smoothed_mut_mean);
hold on
plot(dates(timedate), smoothed_mut_mean(timedate), 'x', 'Linewidth' , 2.8)
...
iii = 1;
load('AsEstRBC_param1.mat', 'stock')

set_param_value('STDERR_AS', parametri_loop(iii, 1));
set_param_value('STDERR_Azc', parametri_loop(iii, 2));
set_param_value('STDERR_Azas', parametri_loop(iii, 3));
set_param_value('STDERR_mu', parametri_loop(iii, 4));
set_param_value('STDERR_GR', parametri_loop(iii, 5));
set_param_value('RHO_zc', parametri_loop(iii, 6));
set_param_value('RHO_zas', parametri_loop(iii, 7));
set_param_value('RHO_AS', parametri_loop(iii, 8));
set_param_value('RHO_MU', parametri_loop(iii, 9));
% Choose time date:
set_param_value('mu', parametri_loop(iii, 10).*(smoothed_series_loop(i_endo_amu,timedate,iii)));
set_param_value('adlag', parametri_loop(iii, 11));
...
var eps_g_as ;  stderr  STDERR_AS * 3.50 ;
...
stoch_simul( periods = 236, order = 1, irf_shocks  = ( eps_g_as ), IRF = 80, nograph, noprint) g_as pas_log growthrate GDP_log Z_log Yas_log; 


store_IRFs_growthrate = zeros (size(parametri_loop,1), options_.irf) ; 
store_IRFs_g_as = zeros (size(parametri_loop,1), options_.irf) ; 
store_IRFs_pas_log = zeros (size(parametri_loop,1), options_.irf) ; 
store_IRFs_GDP_log = zeros (size(parametri_loop,1), options_.irf)  ;
store_IRFs_Z_log = zeros (size(parametri_loop,1), options_.irf)  ;
store_IRFs_Yas_log = zeros (size(parametri_loop,1), options_.irf)  ;

%||||||||||||||||| LOOP ON THE POSTERIOR DRAWS ||||||||||||||||||||
for iii=1:size(parametri_loop,1)
    if mod(iii, 100) == 0
          disp(['  ' num2str(iii) '/   ' num2str(size(parametri_loop,1))]);
    end
    set_param_value('STDERR_AS', parametri_loop(iii, 1));
    set_param_value('STDERR_Azc', parametri_loop(iii, 2));
    set_param_value('STDERR_Azas', parametri_loop(iii, 3));
    ...
    stoch_simul( periods = 236, order = 1, irf_shocks  = ( eps_g_as ), IRF = 80, nograph, noprint) g_as pas_log growthrate GDP_log Z_log Yas_log; 
    if info;
        disp(['Computation fails here  ' num2str(iii)]);
    end;   
    % scale the IRFs to the same std:
    growthrate_eps_g_as = growthrate_eps_g_as  * parametri_loop(iii, 1) / parametri_loop(1, 1);
    g_as_eps_g_as = g_as_eps_g_as  * parametri_loop(iii, 1) / parametri_loop(1, 1);
    pas_log_eps_g_as = pas_log_eps_g_as  * parametri_loop(iii, 1) / parametri_loop(1, 1);
    GDP_log_eps_g_as = GDP_log_eps_g_as  * parametri_loop(iii, 1) / parametri_loop(1, 1);
    Z_log_eps_g_as = Z_log_eps_g_as  * parametri_loop(iii, 1) / parametri_loop(1, 1);
    Yas_log_eps_g_as = Yas_log_eps_g_as  * parametri_loop(iii, 1) / parametri_loop(1, 1);
    
    store_IRFs_growthrate(iii, :)  = growthrate_eps_g_as ;  
    store_IRFs_g_as(iii, :)        = g_as_eps_g_as ;  
    store_IRFs_pas_log(iii, :)     = pas_log_eps_g_as ;  
    store_IRFs_GDP_log(iii, :)     = GDP_log_eps_g_as ;  
    store_IRFs_Z_log(iii, :)       = Z_log_eps_g_as ;  
    store_IRFs_Yas_log(iii, :)     = Yas_log_eps_g_as ;  
end   

growthrate_eps_g_as_mean   = mean(store_IRFs_growthrate, 1) ;
g_as_eps_g_as_mean         = mean(store_IRFs_g_as, 1) ;
pas_log_eps_g_as_mean      = mean(store_IRFs_pas_log, 1) ;
GDP_log_eps_g_as_mean      = mean(store_IRFs_GDP_log, 1) ;
Z_log_eps_g_as_mean        = mean(store_IRFs_Z_log, 1) ;
Yas_log_eps_g_as_mean      = mean(store_IRFs_Yas_log, 1) ;

if higheffect == 1
    % SAVE TO PLOT TOGETHER THE Two SUBSAMPLES
    growthrate_ss_high = growthrate_ss;
    growthrate_high = growthrate_eps_g_as_mean;
    g_as_high = g_as_eps_g_as_mean ;
    pas_log_high = pas_log_eps_g_as_mean ;
    GDP_log_high =  GDP_log_eps_g_as_mean ;
    Z_log_high = Z_log_eps_g_as_mean ;
    Yas_log_high = Yas_log_eps_g_as_mean;
    save('Results\IRF_high_stationary','options_',...
        'growthrate_ss_high' ,'XISS', 'growthrate_high', 'g_as_high',...
        'pas_log_high', 'GDP_log_high', 'Z_log_high', 'Yas_log_high');
elseif higheffect == 0
    growthrate_ss_low = growthrate_ss;
    growthrate_low = growthrate_eps_g_as_mean;
    g_as_low = g_as_eps_g_as_mean ;
    pas_log_low = pas_log_eps_g_as_mean ;
    GDP_log_low =  GDP_log_eps_g_as_mean ;
    Z_log_low = Z_log_eps_g_as_mean ;
    Yas_log_low = Yas_log_eps_g_as_mean;
    save('Results\IRF_low_stationary','options_',...
        'growthrate_ss_low' ,'XISS', 'growthrate_low', 'g_as_low',...
        'pas_log_low', 'GDP_log_low', 'Z_log_low', 'Yas_log_low');
end
