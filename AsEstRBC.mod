...
// KNOWLEDGE LAW OF MOTION (EXISTING TECHNOLOGIES)
...
ZETA0 = ALPHAC/(1/BETTA -1 + DKC)/Xc; 
ZETA1 = ALPHAAS/(1/BETTA -1 + DKAS)/Xas; 
nc = nc_ss;
...
cc = Yc*ccYc;
ucc = 1/cc;
PHISSCPHISSC =  1/nc^(1+NUC) *((1-ALPHAC)/Xc/xwc*ccYc^(-1));
PHISSASPHISSAS = 1/(nas^(1+NUAS)) * (1-ALPHAAS)/Xas/xwas * XISS *(ccYc)^(-1);
