% =================================================================== %
%  This iterates on posterior draws to get the IRFs and compute 
%  fiscal multipliers.
% =================================================================== %

// dynare Fiscal_RBC.mod
...
stoch_simul(  order = 1, irf_shocks  = ( eps_g_as ),  irf  = 160, nograph, noprint) g_as pas Yas Yc growthrate spending_log R;
...
store_IRFs_spending_log = zeros (size(parametri_loop,1), options_.irf)  ;
store_IRFs_R = zeros (size(parametri_loop,1), options_.irf)  ;

aaa = zeros(size(parametri_loop,1),1);
...
for iii=1:size(parametri_loop,1)
    ...
    stoch_simul(  order = 1, irf_shocks  = ( eps_g_as ), irf   = 160, nograph, noprint) growthrate GDP_log spending_log R;
    if info;
    ...
    spending_log_eps_g_as = spending_log_eps_g_as * parametri_loop(iii, 1) / parametri_loop(1, 1);
    R_eps_g_as = R_eps_g_as * parametri_loop(iii, 1) / parametri_loop(1, 1);
    store_IRFs_growthrate(iii, :) = growthrate_eps_g_as;
    store_IRFs_GDP_log(iii, :) =  GDP_log_eps_g_as ;
    store_IRFs_spending_log(iii, :) = spending_log_eps_g_as;
    store_IRFs_R(iii, :) = R_eps_g_as;
    aaa(iii) =  parametri_loop(iii, 10)*smoothed_series_loop(i_endo_amu,timedate,iii); %?

end


if higheffect == 1;
    save('Results\IRFS_multipliers_RBC_1975','store_IRFs_growthrate',...
        'store_IRFs_GDP_log', 'store_IRFs_spending_log'  ,  'store_IRFs_R'  ,'dates','growthrate_ss','options_')
elseif higheffect == 0;
    save('Results\IRFS_multipliers_RBC_2005','store_IRFs_growthrate',...
        'store_IRFs_GDP_log',  'store_IRFs_spending_log'  , 'store_IRFs_R'   ,'dates','growthrate_ss','options_')
end
