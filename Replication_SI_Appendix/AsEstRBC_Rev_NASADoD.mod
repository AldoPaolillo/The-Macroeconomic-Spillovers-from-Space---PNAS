% =================================================================== %
%  Estimate the model with NASA + DoD data.
%  
% =================================================================== %
% close all
...
var
kas   $k_{as}$ (long_name='Capital in NASA/DoD sector')
nas    $n_{as}$ (long_name='Hours in NASA/DoD sector')
...
was    $w_{as}$ (long_name='Wage in NASA/DoD sector')
rkas   $r_{{as}}$  (long_name='Rental rate for capital in NASA/DoD sector')
dwc  $\omega_{c,}$ (long_name='Nominal wage inflation core sector')
dwas  $\omega_{as,}$ (long_name='Nominal wage inflation NASA/DoD sector')
R    $R$         (long_name='Gross interest rate')
r   $r$     
a_zc $a_{z_{c}}$  (long_name='Stationary Productivity in core sector')
a_zas $a_{z_{c}}$  (long_name='Stationary Productivity in NASA/DoD sector')
g_as (long_name='Stationary NASA/DoD demand - percentage of core good production')
pas (long_name='Relative price of NASA/DoD')
Yas    $Y_{as}$ (long_name='NASA/DoD good production')
kas_eff $k_{as}^{eff}$ (long_name='Effective Capital in NASA/DoD sector')
kc_eff $k_{c}^{eff}$ (long_name='Effective Capital in core sector')
Xc $X_{c}$ (long_name='Final core goods markup')
Xas $X_{c}$ (long_name='Final NASA/DoD goods markup')
retail_profits $\Pi_{r}$ (long_name='Retailers profits')
xwc  $X_{{w_c}}$ (long_name='Core sector wage markup')
xwas  $X_{{w_{as}}}$ (long_name='NASA/DoD sector wage markup')
...
dpas (long_name='Inflation of NASA/DoD')
invest
...
data_cc
data_GDP
data_NASADoD % Data on public spending in Space
a_gr
a_mu
GDP 
GDP_log
...
ALPHAAS $\alpha_{AS}$ (long_name='Capital share NASA/DoD sector production')
...
TETAAS $\theta_{\piC}$ (long_name='Calvo parameter for NASA/DoD goods')
LAGPAS $\iota_{\pi c}$ (long_name='Phillips curve NASA/DoD sector indexation parameter')
...
NUAS $\nu_{as}$  (long_name='Labor supply elasticity parameter household to NASA/DoD sector')
...
DKAS $\delta_{kas}$ (long_name='Capital for NASA/DoD good production depreciation')
...
RHO_zas $\rho_{zas}$ (long_name='Persistence NASA/DoD sector productivity')
RHO_S $\rho_{s}$ (long_name='Persistence monetary policy shock')
RHO_AS $\rho_{as}$ (long_name='Persistence NASA/DoD demand')
XISS $\xi_{as}^{ss}$ (long_name='Steady state of cyclical NASA/DoD demand')
phi
epsilon_spill
growthrate_ss
nc_ss
...
// NASA/DoD PRODUCTION = NASA/DoD DEMAND 

Yas  = g_as * Yc ;

// LABOR DEMAND NASA/DoD SECTOR

(1-ALPHAAS)  * Yas/Xas = was * nas;

// NASA/DoD EFFECTIVE CAPITAL DEFINITION

kas_eff =  kas(-1) * exp(-growthrate);

// CAPITAL DEMAND NASA/DoD SECTOR

ALPHAAS *  Yas/Xas = rkas*kas_eff;

// PHILLIPS CURVE NASA/DoD SECTOR
...
// NASA/DoD PRODUCTION DEFINITION
...
//  LABOR SUPPLY TO NASA/DoD SECTOR
...
// WAGE PHILLIPS CURVE NASA/DoD SECTOR
...
// NASA/DoD STATIONARY DEMAND (AS A PERCENTAGE OF GDP)
...
data_GDP = log(GDP) - log(GDP(-1)) + growthrate ;
data_cc = log(cc) - log(cc(-1)) + growthrate ;
data_nc = log(nc) - log(nc(-1)) ;
data_NASADoD = log(Yas) - log(Yas(-1)) + growthrate;
...
stderr eps_mu ,        0.28     , ,       ,  inv_gamma_pdf   , 0.01, 0.01 ; 
stderr eps_gr ,        0.01       , ,       ,  inv_gamma_pdf   , 0.01, 0.01 ;

end;

varobs data_GDP data_cc data_NASADoD;  %NASA-DoD


estimation(
datafile='AerospaceDataset_NASADoD.csv',
optim = ('MaxIter',500),
nograph,
posterior_nograph,
first_obs = 1,
...
data_GDP a_mu xi g_as a_zc a_zas growthrate GDP_log;
shock_decomposition (nograph) growthrate growthrate_prime GDP_growth;


figure
plot(oo_.SmoothedVariables.mu_t  )
legend('\mu')
