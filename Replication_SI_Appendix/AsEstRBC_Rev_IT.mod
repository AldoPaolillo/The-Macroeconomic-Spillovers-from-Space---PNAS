% =================================================================== %
%  Estimate the model with IT data.
%  
% =================================================================== %
% close all

// dynare AsEstRBC_Rev_IT.mod
...
var
kas   $k_{as}$ (long_name='Capital in IT sector')
nas    $n_{as}$ (long_name='Hours in IT sector')
...
was    $w_{as}$ (long_name='Wage in IT sector')
rkas   $r_{{as}}$  (long_name='Rental rate for capital in IT sector')
dwc  $\omega_{c,}$ (long_name='Nominal wage inflation core sector')
dwas  $\omega_{as,}$ (long_name='Nominal wage inflation IT sector')
R    $R$         (long_name='Gross interest rate')
r   $r$     
a_zc $a_{z_{c}}$  (long_name='Stationary Productivity in core sector')
a_zas $a_{z_{c}}$  (long_name='Stationary Productivity in IT sector')
g_as (long_name='Stationary IT demand - percentage of core good production')
pas (long_name='Relative price of IT')
Yas    $Y_{as}$ (long_name='IT good production')
kas_eff $k_{as}^{eff}$ (long_name='Effective Capital in IT sector')
kc_eff $k_{c}^{eff}$ (long_name='Effective Capital in core sector')
Xc $X_{c}$ (long_name='Final core goods markup')
Xas $X_{c}$ (long_name='Final IT goods markup')
retail_profits $\Pi_{r}$ (long_name='Retailers profits')
xwc  $X_{{w_c}}$ (long_name='Core sector wage markup')
xwas  $X_{{w_{as}}}$ (long_name='IT sector wage markup')
...
dpas (long_name='Inflation of IT')
invest
...
ALPHAAS $\alpha_{AS}$ (long_name='Capital share IT sector production')
...
TETAAS $\theta_{\piC}$ (long_name='Calvo parameter for IT goods')
LAGPAS $\iota_{\pi c}$ (long_name='Phillips curve IT sector indexation parameter')
...
NUAS $\nu_{as}$  (long_name='Labor supply elasticity parameter household to IT sector')
...
DKAS $\delta_{kas}$ (long_name='Capital for IT good production depreciation')
...
RHO_zas $\rho_{zas}$ (long_name='Persistence IT sector productivity')
RHO_S $\rho_{s}$ (long_name='Persistence monetary policy shock')
RHO_AS $\rho_{as}$ (long_name='Persistence IT demand')
XISS $\xi_{as}^{ss}$ (long_name='Steady state of cyclical IT demand')
phi
epsilon_spill
growthrate_ss
nc_ss
...
// IT PRODUCTION = IT DEMAND 

Yas  = g_as * Yc ;

// LABOR DEMAND IT SECTOR

(1-ALPHAAS)  * Yas/Xas = was * nas;

// IT EFFECTIVE CAPITAL DEFINITION

kas_eff =  kas(-1) * exp(-growthrate);

// CAPITAL DEMAND IT SECTOR

ALPHAAS *  Yas/Xas = rkas*kas_eff;

// PHILLIPS CURVE IT SECTOR
...
// IT STATIONARY DEMAND (AS A PERCENTAGE OF GDP)
...
data_IT = log(Yas) - log(Yas(-1)) +  growthrate ;
...
varobs data_GDP data_cc data_IT;  %Information Technology Data


estimation(
datafile='AerospaceDataset_wIT.csv', 
optim = ('MaxIter',500),
first_obs = 1,
nobs = 236,
nograph,
